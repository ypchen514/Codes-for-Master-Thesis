...
sys_par_data = [0.1,0.75,0.1,1,1];
    ...
    runcycle = 20; % total sampling times
    n_data_collect = 10000; % cut the output track into # pieces
    tic
    %---Nominal Set------------------------------------------------------
    % give maneuver parameters
    case_name = 'inv_chirp';
    sys_par = [iii,jjj,0.01,1,1]; %need a1, a2, b(better = 0), zoom_rate, zoom_length
    %         sys_par = [iii,jjj];
    sim_time = 30;
    ...
    fraction = 10;% each fraction has 200 samples
    x_open = [];
    y_open = [];
    ax_open = [];
    ay_open = [];
...
toc
GSA_N = [1500,5000,10000,20000,30000,40000,60000,120000];
...
S_data(NUM).GSA_num = N;
S_data(NUM).out_x = S_x_out;
S_data(NUM).out_y = S_y_out;
S_data(NUM).out_yaw = S_yaw_out;
S_data(NUM).index_x = S_index_x;
S_data(NUM).index_y = S_index_y;
S_data(NUM).index_yaw = S_index_yaw;
S_data(NUM).lambda_x = lambda_x;
S_data(NUM).lambda_y = lambda_y;
S_data(NUM).lambda_yaw = lambda_yaw;
