...
% [waypoint, require_velocity] = combined_waypoint(sys_par);
...
for j = 1:desire_piece
    ...
    for p = 1:length(x1_kf)
        in(p) = Simulink.SimulationInput('tadpole_dynamic_open_loop1');%drive open loop vehicle
    ...
    % real model output
    z_ax = interp1(x_open1.time,a_x.signals.values,t_k);
    z_ay = interp1(x_open1.time,a_y.signals.values,t_k);
    z_x = interp1(x_open1.time,x_open.signals.values,t_k);
    z_y = interp1(x_open1.time,y_open.signals.values,t_k);
    z_yaw = interp1(x_open1.time,yaw_rate.signals.values,t_k);
    ...
    for i = 1:length(PHI)
        ...
        save_result(j).x_data = result;
        save_result(j).time = toc;
        clear('result');
end
