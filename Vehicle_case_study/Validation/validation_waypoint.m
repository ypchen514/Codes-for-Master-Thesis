%% Validation waypoint
function [waypoint, require_velocity] = validation_waypoint(sys_par)

%%  Circle
sys_par = [4,1]; % sys_par of circle
r = sys_par(1);
t = linspace(0,0.77,1000);
x = r*cos(2*r*t-pi/2);
y = r*sin(2*r*t-pi/2);
...
%% Double Lane Change
sys_par = [8,0,0,1,1]; % sys_par for DLane
require_velocity = 1;
a1 = sys_par(1);
a2 = sys_par(2);
b = sys_par(3);
...
waypoint_dlane = [x_points(1:300)'+waypoint_circle(end,1) y_points(1:300)'+waypoint_circle(end,2)];
...
waypoint_spline = [x'+waypoint_dlane(end,1) y'+waypoint_dlane(end,2)];
...
waypoint_x = [waypoint_circle(:,1);waypoint_dlane(:,1);waypoint_spline(:,1);waypoint_invchirp(:,1)];
waypoint_y = [waypoint_circle(:,2);waypoint_dlane(:,2);waypoint_spline(:,2);waypoint_invchirp(:,2)];
require_velocity = 1;
